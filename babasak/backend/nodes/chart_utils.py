...
Genie API에서 반환된 DataFrame을 기반으로 프론트엔드용 차트 HTML을 생성합니다.
Genie UI에서는 차트가 자동 렌더링되지만, SDK API 호출 시에는 raw data만 반환되므로
이 모듈이 plotly로 차트를 직접 생성합니다.

차트 스타일: Genie Space와 동일한 스타일 (라인차트, 흰 배경, 확대/축소/팬 지원)

★ 차트 기능 ON/OFF:
    ENABLE_CHART = False 로 바꾸면 차트 생성이 완전히 비활성화됩니다.
    generate_chart_html()가 항상 None을 반환하므로 기존 로직에 영향 없음.
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ═══════════════════════════════════════════════════════════════
# 메인 함수: DataFrame → chart HTML
# ═══════════════════════════════════════════════════════════════

# ═══════════════════════════════════════════════════════════════
# Plotly 툴바 설정 — Genie Space와 동일한 UX
# 확대 (zoom in/out), 축소 (reset), 팬 (드래그), 오토스케일 모두 지원
# ═══════════════════════════════════════════════════════════════
_PLOTLY_CONFIG = {
    "displayModeBar": True,
