...
def missing_price_search_node(state: dict) -> dict:
    ...
    # ── 1순위: 네이버 쇼핑 검색 (LLM 정제로 구조화 dict 받음) ──
    ...
    if backend == "naver":
        try:
            ...
            with mlflow.start_span(name=f"naver_search_[{items_text[:60]}]", span_type=SpanType.TOOL) as span:
                ...
                naver_result = naver_search_structured(items_text)
                span.set_outputs({"result_preview": naver_result.get("text", "")[:500]})

            naver_items = naver_result.get("items", {})
            naver_text = naver_result.get("text", "")
            archive("missing_price_search.naver_attempt", {
                "result_preview": naver_text[:400],
                "num_items": len(naver_items),
            })

            # LLM 정제 결과 기반 분류:
            #   - confidence in {high, medium, low}: cost_calculator가 쓸 수 있음 → found
            #   - confidence == "none": LLM 폴백 대상 → still_missing
            found_items: dict[str, dict] = {}
            found_lines: list[str] = []
            still_missing: list[str] = []
            for ing in unavailable:
                info = naver_items.get(ing) or {}
                ppk = info.get("price_per_kg")
                conf = info.get("confidence", "none")
                if ppk and conf != "none":
                    found_items[ing] = info
                    found_lines.append(
                        f"{ing}: 약 ₩{ppk:,}/kg (네이버 LLM 정제, conf={conf})"
                    )
                else:
            ...
            archive("missing_price_search.naver_validation", {
                ...
                "by_confidence": {
                    c: [n for n, d in found_items.items() if d.get("confidence") == c]
                    for c in ("high", "medium", "low")
                },
            })

            updated_price_info = dict(price_info)
            # 구조화 dict는 cost_calculator가 코드로 매칭할 핵심 데이터
            existing_structured = updated_price_info.get("structured_prices") or {}
            existing_structured = dict(existing_structured)
            existing_structured.update(found_items)
            updated_price_info["structured_prices"] = existing_structured
            # 텍스트도 같이 유지 (report_generator용)
            if found_lines:
